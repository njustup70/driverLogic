...
def generate_launch_description():
    ...
    mid118_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(get_package_share_directory('my_driver'), 'launch', 'mid360_118_bringup.launch.py')
        ),
        launch_arguments={
            'use_rviz': 'false',
        }.items(),
    ...
    slam_main = ExecuteProcess(
        cmd=[
            'bash',
            '-c',
            'python3 ~/ros2_ws/src/MainLogic/Main.py --main-module R1_Main --main-func async_main',
        ],
        output='screen',
        emulate_tty=True,
    )
